...
int check_pre_condition(int row,int col); //전처리검사를 check_pre_condition이라는 함수 사용
...
int check_pre_condition(int row,int col)  //전처리검사를 check_pre_condition이라는 함수를 만듦
{
if(row <= 0 || col <=0) return -1;  //만약 행,열이 0보다 작다면 -1값을 return 하고 아니면 1값을 return하게 만듦
	else return 1;
}
...
int** create_matrix(int row, int col) {  //동적할당을 해주는 함수
	...
	if (check_pre_condition(row,col)==-1) { printf(" 입력값이 잘못되었습니다.\n row,col의 크기를 확인하세요 "); exit(1); } 
     //전처리 검사 함수를 통해 조건을 만족하지 않는다면 exit 를 써서 프로그램 종료
	...
	if(matrix == NULL) {
	printf("melloc error: memory를 할당할 수 없습니다");
	exit(1); 
	}
	return matrix;
}
